...
int main(int argc, char const *argv[])
{
    ...
    // Checking to be executed when re executing child process
    if(strcmp(argv[0],"reExec")==0)
     {
     	
     	printf("\n Re-Executing child process");
     	
        // Getting new socket from the arguments
     	int socket_new = atoi(argv[1]);
    ...
    if(pd==1){
         //message processing
            ...
            int ParentSocket = dup(new_socket);
            
            if(ParentSocket==-1)
           		perror("Creating duplicate socket file descriptor failed");
           
            //convert socket to string
            char socketFileDescriptor[20];
            sprintf(socketFileDescriptor,"%d",ParentSocket);
            char *args[3] = {"reExec",socketFileDescriptor,NULL};
